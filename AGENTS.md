...
- 普通布局、间距、尺寸、字号、圆角、边框、颜色及简单交互状态优先使用 Tailwind，并复用 `web/src/index.css` 的统一 token。
- `@theme` 管理设计尺度，`:root` 管理语义颜色，`@theme inline` 将语义变量映射为工具类；手写 CSS 读取同一组变量，不新增重复别名或独立色板。
- 按页面或模块增量迁移，保持现有尺寸与响应式断点，同批删除被替代的 CSS 属性、状态和媒体查询规则；同一属性只保留一个控制来源。
- SVG 图形、滚动条、`details/summary`、keyframes、Markdown 排版及复杂动态几何可以保留手写 CSS。共享组件尚未迁移的规则继续由组件统一维护。
- 不为覆盖旧样式滥用 `!important`，不通过大量 `@apply` 重建全局样式类；一次性的精确尺寸允许使用任意值类，动态值使用内联样式或 CSS 变量，禁止拼接无法静态识别的 Tailwind 类名。
- 迁移与视觉改版分开；复用已有 UI 组件，只在存在真实重复结构时抽取新组件，不为短工具类组合增加封装。
