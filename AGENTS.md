...
- `src/evaluation/`：Langfuse v4 数据集真实评估、隔离 Runtime、平台回调和轨迹回传；行为测试放在 `src/evaluation/test/`，用法见同目录 README。终端评估必须在数据集 metadata 中显式设置 terminal=true，不读取单个用例的终端开关；执行、同步和评分状态分别记录。
...
- 开发使用 `pnpm`，仓库只维护 `pnpm-lock.yaml`，变更依赖时同步更新该锁文件，不提交 `package-lock.json`。面向用户的快速开始默认展示发布包的使用方式：进入匹配操作系统和 CPU 架构的发布目录后执行 `npm start`，无需安装依赖或重新构建。项目脚本不得硬编码依赖 pnpm；允许打包脚本使用 npm 安装发布包的生产依赖，并使用 npm 验证发布包启动。
...
- 修改前端样式（颜色、间距、字体、布局、动效等纯视觉调整）时不需要测试：不必新增或运行 Vitest 用例，也不需要浏览器验证——不得调用 Codex 内置浏览器及其他浏览器工具，不截图，不做视觉回归检查。如果同一次改动还涉及组件行为、数据或状态逻辑，那部分仍按常规要求补测试。
