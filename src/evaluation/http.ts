...
/** 仅暴露鉴权后的 Experiment 触发接口；不提供本地管理、文件访问和审批能力。 */
export function evaluationWebhook(options: { token: string; projectId: string; start: (input: EvaluationInput) => Promise<{ id: string }> }) {
  return async (request: IncomingMessage, response: ServerResponse): Promise<void> => {
    ...
    try {
      ...
      if (provided.length !== expected.length || !timingSafeEqual(provided, expected)) { send(401, { error: 'Experiment 入口鉴权失败' }); return; }
      ...
      if (Object.keys(config).some(key => !['name'].includes(key))) { send(400, { error: '不支持的 Experiment 配置字段' }); return; }
      if (config.name !== undefined && typeof config.name !== 'string') { send(400, { error: 'Experiment 配置类型不正确' }); return; }
    ...
    } catch { send(400, { error: '无法启动 Experiment，请检查配置或当前运行状态' }); }
  };
}
