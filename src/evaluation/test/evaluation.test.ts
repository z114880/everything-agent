...
async function setup(run?: ReturnType<NonNullable<EvaluationOptions['createRuntime']>>['run']) {
  ...
  const execute = vi.fn<ReturnType<NonNullable<EvaluationOptions['createRuntime']>>['run']>(run ?? (async () => ({ reply: '答案', turnId: 'agent-turn', iterations: 1, stopReason: 'end_turn', toolCallCount: 0, failedToolCallCount: 0, derivedTaskIds: [], model: 'real-model', provider: 'anthropic' as const, ms: 1, retrievalMs: 0, modelMs: 1, toolMs: 0, contextWindow: 10000, contextSafetyTokens: 512, maxTokens: 100, availableInputTokens: 9388, peakEstimatedInputTokens: null, peakInputTokens: null })));
  const close = vi.fn(async () => {});
  const createRuntime: NonNullable<EvaluationOptions['createRuntime']> = () => ({ run: execute, close, createSession: async () => ({ id: 'session', title: '新会话', createdAt: '', updatedAt: '', messageCount: 0, completedTurnCount: 0, incompleteTurnCount: 0 }), listPendingApprovals: () => [], settleApproval: () => false });
