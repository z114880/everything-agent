...
「刷新评分／重试同步」与「连接平台」使用相同的加载反馈：请求期间按钮转圈并禁用，至少显示 300ms；请求完成后显示提示，失败时在页面内展示错误。未配置 Langfuse 时，连接平台失败若与轮询已常驻展示的连接错误相同，则改用浮层再提示一次，不覆盖 alert 区，避免同一条错误出现「Error: 缺少…」与「缺少…」两种文案导致页面漂移。刷新请求完成不代表已经收到评分或全部同步成功，具体结果以记录中的状态为准。
...
平台侧的一次性配置步骤（回调地址、Default config、Custom headers，以及“复制 authorization 值”）收在「数据集与实验」分区描述下方的「从 Langfuse 管理平台触发 Experiment」弹窗里，页面不再常驻长说明；弹窗底部的复制按钮同样只在本地入口已配置时可用。
