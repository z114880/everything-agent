...
- Current Session 来自服务端 SQLite 当前会话的压缩检查点与后续已完成消息，不包含本轮新输入；最近 3 个已完成回合另供检索意图判断。
...
- 当前输入、会话历史、System Prompt、Tool Schemas 和召回证据进入 Working Memory，检查上下文预算后交给模型推理；达到输入额度 70% 时先经过 Compact 节点，以 30% 为软目标；工具结果完整返回后也检查同一阈值，再回到模型，最终回复用户。Tool Schemas 表示本轮注册工具的名称、用途和参数结构，不属于 Procedural Memory。
- Compact 相关连线统一为 Working Memory → Compact → LLM；首轮与工具返回后的压缩事件均沿此路径展示。
