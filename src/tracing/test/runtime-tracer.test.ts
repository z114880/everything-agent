...
it("运行结束前就导出步骤，共享本地事件标识，删除 JSONL 也不影响实时上传", async () => {
  ...
  try {
    await tracer.record("turn_started", { turnId: "r", sessionId: "s", userInput: "私人问题" });
    await tracer.record("model_request", { turnId: "r", modelCallId: "m", model: "test" });
    await tracer.record("model_response", { turnId: "r", modelCallId: "m", model: "test", tokenUsage: { inputTokens: 3, outputTokens: 2 } });
    const records = await readTraceRecords(home);
    expect(records.map(record => record.sequence)).toEqual([1, 2, 3]);
    expect(records.some(record => record.type === "turn_completed")).toBe(false);
...
it("网络阻塞不阻止 JSONL 落盘，关闭等待在途导出且失败仅记录一次本地错误", async () => {
  ...
  const tracer = createRuntimeTracer(home);
  await tracer.record("turn_started", { turnId: "r" });
  await tracer.record("turn_completed", { turnId: "r" });
  await started.promise;
  expect((await readTraceRecords(home)).map(record => record.type)).toEqual(["turn_started", "turn_completed"]);
...
it("禁用时仅写本地，错误配置有本地记录且不阻断任务", async () => {
  ...
  await disabled.record("turn_completed", { turnId: "disabled" }); await disabled.close();
  vi.stubEnv("LANGFUSE_BASE_URL", "http://name:secret@localhost");
  const invalid = createRuntimeTracer(home);
  await invalid.record("turn_completed", { turnId: "invalid" }); await invalid.close();
  expect(fetcher).not.toHaveBeenCalled();
  const records = await readTraceRecords(home);
  expect(records.filter(record => record.type === "turn_completed")).toHaveLength(2);
...
it("未启用的服务端配置不产生网络请求", async () => {
  ...
  const tracer = createRuntimeTracer(home);
  await tracer.record("turn_completed", { turnId: "r" }); await tracer.close();
...
it("无会话的向量操作按重建 ID 或操作 ID 归组，生命周期不被拆成不同运行", async () => {
  ...
  await tracer.close();
  const records = await readTraceRecords(home);
  expect(records.filter(record => record.traceId === "rebuild")).toHaveLength(4);
  expect(records.filter(record => record.traceId === "standalone")).toHaveLength(2);
