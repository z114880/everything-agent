...
import { JsonlTracer, readTraceRecords } from "../jsonl-tracer.ts";
...
describe("JSONL 运行记录", () => {
  it("写入可用于 eval 的完整模型请求与结构化工具结果", async () => {
    ...
    expect(await readFile(join(home, "traces", "2026-09-03", "s1.jsonl"), "utf8"))
  ...
  it("按日期目录与 Session 文件隔离记录", async () => {
    ...
    expect((await readdir(dateDirectory)).sort()).toEqual(["s1.jsonl", "s2.jsonl", "system.jsonl"]);
  });
  ...
  it("发现损坏的当日文件时切换到恢复文件", async () => {
    ...
    await writeFile(join(directory, "s1.jsonl"), "broken\n", "utf8");
    const warning = vi.fn();
    ...
    const recovered = (await readdir(directory)).find((file) => file.startsWith("s1.recovered-"));
  ...
  it("目录不存在时返回空数组，损坏行转换为可见错误", async () => {
    ...
    const directory = join(home, "traces", "2026-09-03");
    await mkdir(directory, { recursive: true });
    await writeFile(join(directory, "s1.jsonl"), '{"version":1,"type":"turn_end","timestamp":"2026-09-03T08:00:00Z","runId":"r1"}\n损坏\n', "utf8");
    expect(await readTraceRecords(home, 100)).toEqual([
      expect.objectContaining({ type: "turn_end", runId: "r1" }),
      expect.objectContaining({ type: "trace_read_error", payload: { file: "2026-09-03/s1.jsonl" } }),
    ]);
  });
