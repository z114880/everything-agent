...
export async function listTraceRuns(home: string, options: TracePageOptions = {}): Promise<TraceRunPage> {
  ...
  for (const { records } of await readTraceFiles(home)) for (const record of records) {
    const value = grouped.get(record.runId) ?? { runId: record.runId, startedAt: new Date(record.timestamp).toISOString(), eventCount: 0, status: "running" };
    const timestamp = new Date(record.timestamp).toISOString();
    value.startedAt = value.startedAt < timestamp ? value.startedAt : timestamp;
    value.eventCount++;
    if (["run_completed", "run_failed", "memory_task_completed", "memory_task_failed", "consolidation_completed", "consolidation_failed"].includes(record.type)) value.status = record.type.endsWith("failed") ? "failed" : "completed";
