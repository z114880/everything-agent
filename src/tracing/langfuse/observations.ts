...
export class ObservationMapper {
  ...
  constructor(privateContext: (record: TraceRecord) => ObservationContext, captureContent = false) { this.context = privateContext; this.captureContent = captureContent; }
  ...
  accept(record: TraceRecord): Observation | null {
    ...
    if (spec?.[2] === "start") {
      ...
      if (this.starts.size >= 4096) this.starts.delete(this.starts.keys().next().value!);
      this.starts.set(key, record);
      if (["run", "task", "consolidation"].includes(spec[1])) {
        if (this.roots.size >= 4096) this.roots.delete(this.roots.keys().next().value!);
        this.roots.set(record.runId, identifier(key, 16));
      }
      return null;
    }
  ...
  private key(r: TraceRecord, family: string): string {
    return `${r.runId}:${family}:${(family === "tool" ? r.toolCallId : r.modelCallId ?? r.operationId) ?? (family === "batch" ? r.payload?.batchIndex : "") ?? ""}:${r.payload?.attempt ?? ""}`;
  }
  private convert(start: TraceRecord, end: TraceRecord, type: string, family: string, key: string): Observation {
    ...
    const root = ["run", "task", "consolidation"].includes(family);
    const runRootId = this.roots.get(end.runId);
    const spanId = root ? identifier(key, 16) : identifier(type === "event" ? `${end.runId}:${end.eventId ?? end.sequence ?? end.timestamp}:${end.type}` : key, 16);
    const parent = root ? context.rootId : end.toolCallId && !end.type.startsWith("tool_") ? identifier(this.key(end, "tool"), 16) : runRootId ?? context.rootId;
    ...
    const error = end.type.endsWith("failed") || end.type.endsWith("retry") || end.type === "observation_incomplete" || payload.isError === true || Boolean(payload.errorType);
