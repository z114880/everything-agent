...
export interface LangfuseConfiguration {
  ...
  evaluationCaptureContent: boolean;
}
...
export function readLangfuseConfiguration(home: string): LangfuseConfiguration | null {
  ...
  const baseUrl = env.LANGFUSE_BASE_URL ?? "http://localhost:3000";
  ...
  return { baseUrl: baseUrl.replace(/\/+$/, ""), publicKey: env.LANGFUSE_PUBLIC_KEY, secretKey: env.LANGFUSE_SECRET_KEY, projectId: env.LANGFUSE_PROJECT_ID, captureContent: env.LANGFUSE_CAPTURE_CONTENT === "true", evaluationCaptureContent: env.LANGFUSE_EVALUATION_CAPTURE_CONTENT === "true" };
}
