...
it.each([false, true])("Runtime 同时记录前台和后台，导出故障=%s 不改变任务结果", async (failExport) => {
  ...
  const runtime = createAgentRuntime({ home, defaultSystemPromptPath: join(home, "EVERYTHING.md") }, { automaticConsolidation: false });
  try {
    ...
    const recordedSkills = traces.flatMap((file) => file.records).filter((record) => record.type === "skill_loaded");
