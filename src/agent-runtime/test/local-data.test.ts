...
describe("本地 Agent 数据清理", () => {
  it("删除数据库、会话附件和全部 trace，保留 .env、langfuse.env、EVERYTHING.md、Skills 和 config.json", async () => {
    ...
    expect((await readdir(home)).sort()).toEqual([".env", "EVERYTHING.md", "config.json", "langfuse.env", "skills"]);
    expect(await readFile(join(home, ".env"), "utf8")).toBe('EVERYTHING_AGENT_API_KEY="keep"\n');
    expect(await readFile(join(home, "langfuse.env"), "utf8")).toBe('LANGFUSE_ENABLED=true\nLANGFUSE_SECRET_KEY="keep"\n');
