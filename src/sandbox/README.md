...
Sandbox 为终端类工具提供**由内核强制**的执行边界。它只负责「命令能碰到什么」，不负责「该不该执行这条命令」——后者属于审批策略。
...
策略写错时的失效是静默的，因此 macOS 上的用例直接调用真实 `sandbox-exec` 验证边界，而非断言生成的文本。bubblewrap 的真实边界只能在 Linux 上验证，本模块在其他平台只覆盖参数组装与缺少 `bwrap` 时的失败路径。
