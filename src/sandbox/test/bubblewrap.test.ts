...
describe("bubblewrap 参数组装", () => {
  const isDirectory = (path: string) => path.endsWith(".ssh");

  it("全盘只读挂载排在最前，作为其余挂载的底", () => {
    const args = buildBubblewrapArgs(policy, isDirectory);
  ...
  it("默认切断网络命名空间，放行时不再隔离", () => {
    expect(buildBubblewrapArgs(policy, isDirectory)).toContain("--unshare-net");
    expect(buildBubblewrapArgs({ ...policy, allowNetwork: true }, isDirectory)).not.toContain("--unshare-net");
  });
  ...
  it("默认目录判定不依赖注入即可工作", () => {
    expect(buildBubblewrapArgs({ ...policy, denyRead: ["/nonexistent-path-for-test"] })).toContain("--ro-bind");
  });
