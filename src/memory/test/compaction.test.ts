...
it("压缩检查点跨重启生效，完整聊天与最近原始回合保留，完成时不重复写入工具消息", async () => {
  ...
  memory.startTurn(session.id, "old", "历史请求原文");
  await memory.completeTurn(session.id, "old", [answer("历史回答原文")]);
  memory.startTurn(session.id, "current", "当前请求");
  ...
  await memory.completeTurn(session.id, "current", [...tools, answer("最终回答")]);
  ...
  expect(memory.getChatLog(session.id).find((entry) => entry.turnId === "current")?.compactions).toMatchObject([{ compactionId: compacted.compactionId, beforeTokens: 7000, afterTokens: 2000 }]);
  ...
  reopened.startTurn(session.id, "next", "下一轮");
  await reopened.completeTurn(session.id, "next", [answer("下一答")]);
...
it("保存失败时回滚原始消息和检查点，不覆盖此前成功结果", async () => {
  ...
  expect(() => memory.saveCompaction(session.id, "missing", [], checkpoint([]))).toThrow("回合的用户消息不存在");
});

it("压缩后可分页回查当前会话的原文，包含已保存工具结果并排除新输入", async () => {
  const { memory } = await setup();
  const session = memory.createSession();
  memory.startTurn(session.id, "old", "必须保留的精确编号 ABC123");
  await memory.completeTurn(session.id, "old", [answer("收到")]);
  memory.startTurn(session.id, "active", "继续工作");
  const tools: AgentMessage[] = [
  ...
  memory.startTurn(session.id, "new", "未压缩的新输入");
  ...
  expect(entries.map((entry) => entry.turnId)).not.toContain("new");
