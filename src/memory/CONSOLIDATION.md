...
- 自动触发：按服务端本地时区的自然日，在用户当天首次进入 Agent 页面时检查；仅当 Semantic Memory 非空时后台入队，当前使用场景为北京时间。
- 服务端兜底触发：页面挂载检查无法覆盖长期挂机的进程，因此 Runtime 启动后按本地时间轮询，当天 15:00 之后补发一次 daily 检查。轮询比较当前本地时间而不是一次性定时到点，休眠唤醒、进程中途启动和系统时钟调整都会补跑当天检查；本进程当天已检查过就不再重复，跨自然日重新具备资格。
- 自动整理每天最多创建一个任务。两条自动路径共用同一份每日去重记录，日期与触发记录持久化，刷新页面、多标签页、重复请求和服务重启不重复创建。
- Agent 页面提供英文按钮 **Consolidate**。手动触发不受每日自动次数限制，可以在自动整理完成后再次执行；Semantic Memory 为空时按钮不可点击，服务端仍会对空库请求返回跳过且不创建任务。
...
1. 入队时原子检查已有任务、Semantic Memory 是否非空及每日触发记录；空库不创建任务或占用每日配额，实际入队时保存触发来源（自动或手动）。
2. 执行时读取全量 semantic facts，固定事实 ID 集合；各批次读取最新正文并记录全库版本。已入队任务若在执行前被其他操作清空，仍可安全完成且不调用模型。
3. 计算上下文预算。完整请求预算包含系统提示、事实正文、必要元数据、结构化输出约束和输出预留。
4. 能放入一次请求时，将全部事实提交给模型；放不下时拆成两个或多个可恢复子任务，覆盖所有事实。
5. 模型返回结构化整理建议；有实际操作时返回 `update/merge/delete`，无操作时必须返回 `noop` outcome，其中无变化使用 `no_change`，只有未解决冲突使用 `unresolved_conflict`。代码验证目标 ID、事实版本、操作类型和合并内容，再执行变更。模型不能直接操作数据库。
6. 保存子任务检查点和操作凭据，汇总变更计数、未解决冲突和错误，更新任务状态。

Consolidation 继续使用后台记忆任务的 Agent Model 依赖与持久化执行能力。它与聊天中的单条记忆写入在产品职责上独立；底层可以复用队列、校验和事务提交机制。Small Model 仅用于 retrieval gate。
...
- 未解决冲突保存审查计数，并以 `noop/unresolved_conflict` 记录本批次跳过结果；不保存正文或独立冲突档案。同一关联在多批次出现时可能重复计数。
- 子任务提供跨组共同审查机会，不能保证模型必然识别所有语义重复或冲突。
- 模型输出按首尾花括号截取后解析，允许 JSON 外的 markdown 围栏与说明文字；只有缺失或残缺的 JSON 才失败重试。
- 失败最多执行三次，间隔 1 秒和 2 秒；自动配额按任务创建占用，失败后可手动重试。
- 自动检查遇到已有手动或跨日任务时复用该任务并记录当天检查，避免完成后又自动创建任务。
- 兜底检查只负责按时发起，是否入队仍由模型配置、Semantic Memory 是否为空和每日去重决定；发起过程本身失败记录 `daily_consolidation_check_failed`，整理任务自身的失败仍由后台队列记录。
- 未配置模型或 Semantic Memory 为空时，自动检查不创建任务、不占用每日配额；已入队任务执行时遇到空库则直接完成，不调用模型。
