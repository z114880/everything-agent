...
export function EvaluationPage() {
  ...
  return <div className="p-6 space-y-6 max-w-[1440px] mx-auto w-full overflow-auto">
    ...
    <section className="rounded-xl border p-5 space-y-4">
      <div className="flex items-center justify-between gap-4"><div><h2 className="font-semibold">Langfuse 连接</h2><p className="text-sm text-muted-foreground">{connected ? '平台连接正常' : data?.configured ? '本地入口已就绪，点击连接平台检查' : '等待配置'} · {data?.baseUrl}</p></div>
    ...
    <section className="rounded-xl border p-5 space-y-4">{!run ? <p className="text-sm text-muted-foreground">选择实验查看用例详情。</p> : <>
