...
export function MemoryPage() {
  ...
  return <div className="content-wrap memory-page">
    ...
    {tab === "consolidation" && <div className="panel table-scroll"><table><thead><tr><th>Status</th><th>Session ID</th><th>Trigger</th><th>High Watermark</th><th>Facts</th><th>时间</th></tr></thead><tbody>{data.consolidations.map((item) => <tr key={item.id}><td><span className={"status-pill " + item.status}>{item.status}</span>{item.errorType && <small>{item.errorType}</small>}</td><td><code>{short(item.sessionId)}</code></td><td>{item.trigger}</td><td>{item.throughMessageId}</td><td>+{item.factsCreated} / ~{item.factsUpdated} / ={item.factsSkipped}</td><td>{local(item.startedAt)}</td></tr>)}</tbody></table></div>}
  </div>;
}
