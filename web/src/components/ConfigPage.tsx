...
export function ConfigPage() {
  ...
  const [smallProvider, setSmallProvider] = useState<AgentProvider>("anthropic");
  ...
  const [sessionSearchWindow, setSessionSearchWindow] = useState<NumericInputValue>(5);
  const [sessionScrollStep, setSessionScrollStep] = useState<NumericInputValue>(10);
  const [sessionRecallMessageLimit, setSessionRecallMessageLimit] = useState<NumericInputValue>(100);
  const [sessionRecallTokenLimit, setSessionRecallTokenLimit] = useState<NumericInputValue>(8_192);
  const [modelContextWindow, setModelContextWindow] = useState<NumericInputValue>(32_768);
  const [retrievalMode, setRetrievalMode] = useState<RetrievalMode>("lexical_only");
  const [embeddingBaseUrl, setEmbeddingBaseUrl] = useState("");
  const [embeddingModel, setEmbeddingModel] = useState("");
  const [embeddingMinimumSimilarity, setEmbeddingMinimumSimilarity] = useState<NumericInputValue>(0.30);
  ...
  const [models, setModels] = useState<Record<"agentModel" | "smallModel", string[]>>({ agentModel: [], smallModel: [] });
  const [savingSection, setSavingSection] = useState<ConfigSection | null>(null);
  ...
  async function saveSection(section: ConfigSection, force = false) {
    if (!settings) return;
    if (section === "runtime") setRuntimeMessage("");
    setSavingSection(section);
    setModelMessage(force ? "正在强制保存模型连接…" : `正在保存${sectionLabel(section)}…`);
    setForceSection(null);
    try {
      const result = await saveAgentConfig({
        agentModel: section === "model"
          ? { provider: agentProvider, model: agentModel, baseUrl: agentBaseUrl, apiKey: agentApiKey, clearApiKey: false }
          : savedModelInput(settings.agentModel),
        smallModel: section === "model"
          ? { provider: smallProvider, model: smallModel, baseUrl: smallBaseUrl, apiKey: smallApiKey, clearApiKey: false }
          : savedModelInput(settings.smallModel),
        force,
        sessionSearchWindow: section === "runtime" ? requiredNumericValue(sessionSearchWindow, "Session Search Window") : settings.sessionSearchWindow,
        sessionScrollStep: section === "runtime" ? requiredNumericValue(sessionScrollStep, "Session Scroll Step") : settings.sessionScrollStep,
        sessionRecallMessageLimit: section === "runtime" ? requiredNumericValue(sessionRecallMessageLimit, "Recall Message Limit") : settings.sessionRecallMessageLimit,
        sessionRecallTokenLimit: section === "runtime" ? requiredNumericValue(sessionRecallTokenLimit, "Recall Token Limit") : settings.sessionRecallTokenLimit,
        modelContextWindow: section === "runtime" ? requiredNumericValue(modelContextWindow, "Model Context Window") : settings.modelContextWindow,
        retrievalMode: section === "retrieval" ? retrievalMode : settings.retrievalMode,
        embeddingBaseUrl: section === "retrieval" ? embeddingBaseUrl : settings.embeddingBaseUrl,
        embeddingModel: section === "retrieval" ? embeddingModel : settings.embeddingModel,
        embeddingQueryTemplate: settings.embeddingQueryTemplate,
        embeddingDocumentTemplate: settings.embeddingDocumentTemplate,
        embeddingMinimumSimilarity: section === "retrieval" ? requiredNumericValue(embeddingMinimumSimilarity, "Minimum Similarity") : settings.embeddingMinimumSimilarity,
      ...
      const modelCount = section === "model" ? result.models.agentModel.length + result.models.smallModel.length : 0;
      setModelMessage(modelCount
        ? `${sectionLabel(section)}保存成功，连接测试返回 ${modelCount} 个模型。`
        : `${sectionLabel(section)}保存成功，下一回合立即生效。`);
    } catch (error) {
  ...
  async function resetRuntime() {
    setModelMessage("");
    setRuntimeMessage("正在恢复默认运行值…");
    try {
      const result = await resetRuntimeConfig();
      setSettings(result.settings);
      applyRuntimeInputs(result.settings);
      setRuntimeMessage("运行配置已恢复默认值；模型连接和 EVERYTHING.md 未修改。");
    } catch (error) {
  ...
  return (
    <div className="content-wrap config-page">
      <PageHeading eyebrow="本地运行 / 安全配置" title="配置中心" description="管理模型连接、记忆检索与运行边界。" />
      ...
      <div className="config-grid">
        <Card className="config-card config-model-card">
          <CardHeader className="config-card-header">
            ...
            <div><CardTitle>模型连接</CardTitle><CardDescription>Agent Model 与 Small Model 使用完全独立的连接配置。</CardDescription></div>
            <Badge variant="outline">热更新</Badge>
          </CardHeader>
          <CardContent className="config-card-content">
            <div className="config-form-grid">
              <div className="config-field-wide"><strong>Agent Model</strong><span className="field-help">负责主 Agent 推理、工具调用、记忆写入与 consolidation。</span></div>
              <ConfigField label="Provider">
                <Select value={agentProvider} onValueChange={(value) => { setAgentProvider(value as AgentProvider); setForceSection(null); }}>
                  <SelectTrigger aria-label="Agent Model Provider"><SelectValue /></SelectTrigger>
              ...
              <ConfigField label="Model">
                <Input value={agentModel} onChange={(event) => setAgentModel(event.target.value)} list="agent-model-list" placeholder="输入 Agent 模型 ID" />
                <datalist id="agent-model-list">{models.agentModel.map((value) => <option key={value} value={value} />)}</datalist>
              </ConfigField>
              <ConfigField label="Base URL"><Input value={agentBaseUrl} onChange={(event) => setAgentBaseUrl(event.target.value)} placeholder={agentProvider === "anthropic" ? "https://api.anthropic.com" : "https://api.openai.com/v1"} /></ConfigField>
              <ConfigField label="API Key" help={agentKeyKnown && settings?.agentModel.keyConfigured ? `已配置 ····${settings.agentModel.keyLast4}` : "尚未配置"} icon={<KeyRound size={13} />}>
                <Input type="password" value={agentApiKey} onChange={(event) => setAgentApiKey(event.target.value)} placeholder={agentKeyKnown && settings?.agentModel.keyConfigured ? "留空以保留已保存的值" : "输入 Agent Model API Key"} />
              </ConfigField>
              <div className="config-field-wide"><strong>Small Model</strong><span className="field-help">仅负责 retrieval gate。</span></div>
              <ConfigField label="Provider">
                <Select value={smallProvider} onValueChange={(value) => { setSmallProvider(value as AgentProvider); setForceSection(null); }}>
                  <SelectTrigger aria-label="Small Model Provider"><SelectValue /></SelectTrigger>
                  <SelectContent><SelectItem value="anthropic">Anthropic</SelectItem><SelectItem value="openai-compatible">OpenAI Compatible</SelectItem></SelectContent>
                </Select>
              </ConfigField>
              <ConfigField label="Model">
                <Input value={smallModel} onChange={(event) => setSmallModel(event.target.value)} list="small-model-list" placeholder="输入 Small 模型 ID" />
                <datalist id="small-model-list">{models.smallModel.map((value) => <option key={value} value={value} />)}</datalist>
              </ConfigField>
              <ConfigField label="Base URL"><Input value={smallBaseUrl} onChange={(event) => setSmallBaseUrl(event.target.value)} placeholder={smallProvider === "anthropic" ? "https://api.anthropic.com" : "https://api.openai.com/v1"} /></ConfigField>
              <ConfigField label="API Key" help={smallKeyKnown && settings?.smallModel.keyConfigured ? `已配置 ····${settings.smallModel.keyLast4}` : "尚未配置"} icon={<KeyRound size={13} />}>
                <Input type="password" value={smallApiKey} onChange={(event) => setSmallApiKey(event.target.value)} placeholder={smallKeyKnown && settings?.smallModel.keyConfigured ? "留空以保留已保存的值" : "输入 Small Model API Key"} />
              </ConfigField>
            </div>
            <div className="config-card-actions"><Button onClick={() => void saveSection("model")} disabled={savingSection !== null || !settings || !agentModel.trim() || !smallModel.trim()}><Save size={14} />{savingSection === "model" ? "正在保存…" : "保存模型连接配置"}</Button>{forceSection === "model" && <Button variant="destructive-outline" onClick={() => void saveSection("model", true)} disabled={savingSection !== null}>仍然保存</Button>}{(settings?.agentModel.keyConfigured || settings?.smallModel.keyConfigured) && <ModelKeysClearDialog disabled={clearingModelKeys} onConfirm={() => void clearSavedModelApiKeys()} />}</div>
          </CardContent>
        </Card>
          ...
          <CardContent className="config-card-content">
            <div className="config-form-grid">
              ...
              <ConfigField label="Minimum Similarity" labelSuffix={<SimilarityHelp />}>
                <Input type="number" min={-1} max={1} step={0.05} value={embeddingMinimumSimilarity} onChange={(event) => setEmbeddingMinimumSimilarity(parseNumericInput(event.target.value))} />
              </ConfigField>
            ...
            <div className="config-index-status">
              <div className="config-index-copy">
                <strong>向量索引</strong>
                <span>{settings?.embeddingIndex.ready
                  ? `当前配置已就绪 · ${settings.embeddingIndex.generationId?.slice(0, 8)}`
                  : settings?.embeddingIndex.generationId ? `配置待重建 · 旧索引 ${settings.embeddingIndex.generationId.slice(0, 8)} 仍可用` : "尚未建立索引"}</span>
              </div>
        ...
        <Card className="config-card config-runtime-card">
          ...
          <CardContent className="config-card-content">
            <div className="config-runtime-grid">
              <ConfigField label="Session Search Window" help="命中点初始单侧窗口，默认 5。"><Input type="number" min={settings?.limits.sessionSearchWindow?.min ?? 1} max={settings?.limits.sessionSearchWindow?.max ?? 20} value={sessionSearchWindow} onChange={(event) => setSessionSearchWindow(parseNumericInput(event.target.value))} /></ConfigField>
              <ConfigField label="Session Scroll Step" help="每次完整扩窗的单侧增量，默认 10。"><Input type="number" min={settings?.limits.sessionScrollStep?.min ?? 1} max={settings?.limits.sessionScrollStep?.max ?? 50} value={sessionScrollStep} onChange={(event) => setSessionScrollStep(parseNumericInput(event.target.value))} /></ConfigField>
              <ConfigField label="Recall Message Limit" help="单次最多返回条目数，默认 100。"><Input type="number" min={settings?.limits.sessionRecallMessageLimit?.min ?? 1} max={settings?.limits.sessionRecallMessageLimit?.max ?? 200} value={sessionRecallMessageLimit} onChange={(event) => setSessionRecallMessageLimit(parseNumericInput(event.target.value))} /></ConfigField>
              <ConfigField label="Recall Token Limit" help="估算 token 预算，默认 8,192。"><Input type="number" min={settings?.limits.sessionRecallTokenLimit?.min ?? 256} max={settings?.limits.sessionRecallTokenLimit?.max ?? 131072} value={sessionRecallTokenLimit} onChange={(event) => setSessionRecallTokenLimit(parseNumericInput(event.target.value))} /></ConfigField>
              <ConfigField label="Model Context Window（tokens）" help="默认 32,768，并预留 2,048 output tokens 与 512-token 安全余量。"><Input type="number" min={settings?.limits.modelContextWindow?.min ?? 4096} max={settings?.limits.modelContextWindow?.max ?? 2000000} value={modelContextWindow} onChange={(event) => setModelContextWindow(parseNumericInput(event.target.value))} /></ConfigField>
            </div>
            <div className="config-card-actions"><Button onClick={() => void saveSection("runtime")} disabled={savingSection !== null || !settings || [sessionSearchWindow, sessionScrollStep, sessionRecallMessageLimit, sessionRecallTokenLimit, modelContextWindow].includes("")}><Save size={14} />{savingSection === "runtime" ? "正在保存…" : "保存运行参数"}</Button><Button onClick={() => void resetRuntime()}><RotateCcw size={14} />恢复默认运行值</Button>{runtimeMessage && <span className="config-action-message" role="status">{runtimeMessage}</span>}</div>
          </CardContent>
        </Card>

        {modelMessage && <Alert className="config-feedback" variant={forceSection ? "warning" : "default"}><Info /><AlertDescription>{modelMessage}</AlertDescription></Alert>}
...
function savedModelInput(value: AgentSettings["agentModel"]) {
  return { provider: value.provider, model: value.model, baseUrl: value.baseUrl, apiKey: "", clearApiKey: false };
}
