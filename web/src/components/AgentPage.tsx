...
export function AgentPage({ onOpenConfig }: AgentPageProps) {
  ...
  const edgePlaybackRef = useRef<ReturnType<typeof createEdgePlayback> | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeBackgroundEvents((kind, event) => {
      ...
      if (
        ...
        playback.show([]);
      if (kind === "memory_task_completed" || kind === "consolidation_completed")
        void refreshSemanticCount().catch((error) =>
          setConsolidationStatus(
  ...
  async function refreshConsolidation() {
    const task = await memoryAction<{
      status: string;
      errorType: string | null;
    } | null>({ action: "consolidation_status" });
  ...
  async function refreshSemanticCount() {
    const count = await memoryAction<number>({ action: "semantic_count" });
  ...
  async function consolidate(trigger: "daily" | "manual") {
    if (trigger === "manual") setConsolidating(true);
    try {
      const task = () =>
        memoryAction<{
          status?: string;
          reason?: string;
        } | null>({ action: "consolidate", trigger });
      const result = trigger === "manual"
        ? await withMinimumDuration(task)
        : await task();
      if (
        result?.status === "skipped" &&
        result.reason === "no_semantic_memory"
      ) {
        setConsolidating(false);
        setSemanticCount(0);
  ...
  async function selectSession(sessionId: string, knownSessions = sessions) {
    ...
    const result = await memoryAction<{
      ...
      sessions: SessionSummary[];
    }>({ action: "select_session", sessionId });
  ...
  async function createSession() {
    ...
    try {
      const result = await memoryAction<{
        session: SessionSummary;
        sessions: SessionSummary[];
      }>({ action: "create_session", previousSessionId: activeSessionId });
  ...
  async function renameActiveSession() {
    ...
    setSessions((current) =>
      current.map((item) =>
        item.id === activeSessionId ? { ...item, title: title.trim() } : item,
      ),
    );
  }
  ...
  async function deleteActiveSession() {
    ...
    if (result.sessions.length) {
      setSessions(result.sessions);
      await selectSession(result.sessions[0]!.id, result.sessions);
    } else {
      const created = await memoryAction<{
        session: SessionSummary;
        sessions: SessionSummary[];
  ...
  async function send() {
    ...
    setNodeStates({
      ...
      ...idleStates,
      user_prompt: "running",
      session_chat_history: "running",
      everything_md: "running",
      skills_catalog: "running",
      procedural_memory: "running",
    });
    edgePlayback.show([
    ...
    try {
      ...
      const refreshed = await memoryAction<{
        messages: ChatLogEntry[];
        sessions: SessionSummary[];
      }>({ action: "select_session", sessionId: activeSessionId });
          ...
          actions={
            <div className="agent-intro-actions">
              ...
              >
                ...
                <Button
                  className="consolidation-button"
                  variant="secondary"
                  size="sm"
                  ...
                  onClick={() => void consolidate("manual")}
                >
        ...
        <div className="chat-pane">
          ...
          >
            <div
              ...
              onKeyDown={(event) => {
                if (event.key === "Escape" && !sessionRailCollapsed) {
                  setSessionRailCollapsed(true);
            ...
            >
              <div className="chat-toolbar">
                <Button
                  variant="outline"
                  size="sm"
                  className="session-toolbar-button new-session"
            ...
            <div className="agent-composer">
              <div className="composer-input-box">
                <Textarea
                  ...
                  onKeyDown={(event) => {
                    if (shouldSubmitAgentComposer(event)) {
  ...
  return (
    <div className={`assistant-card ${message.error ? "has-error" : ""}`}>
      <div className="assistant-stages">
        <Badge
          variant={message.pending ? "default" : "success"}
          className={message.pending ? "stage-chip active" : "stage-chip done"}
        >
          loop
        </Badge>
        {message.tools.map((tool) => (
          <Badge
            ...
            key={tool.id}
            className={`stage-chip ${tool.status}`}
          >
            <Wrench size={10} /> {tool.name}
          </Badge>
        ))}
        <Badge
          variant={message.content ? "success" : "outline"}
          className={message.content ? "stage-chip done" : "stage-chip"}
        >
          reply
        </Badge>
      </div>
      ...
      {message.tools.map((tool) => (
        ...
        >
          ...
          <pre>
            {JSON.stringify({ args: tool.args, output: tool.output }, null, 2)}
          </pre>
        </details>
      ))}
      {message.error ? (
      ...
      <div className="assistant-meta">
        ...
        {message.result && (
          <>
            {" "}
            · {message.result.iterations} iter · {message.result.model}
          </>
        )}
      </div>
    </div>
...
function applyAgentEvent(
  ...
  setNodeStates: React.Dispatch<
    React.SetStateAction<Record<string, VisualNodeState>>
  >,
  ...
  if (kind === "model_response")
    setNodeStates((states) => ({ ...states, llm: "done" }));
  if (kind === "model_failed")
    setNodeStates((states) => ({ ...states, llm: "error" }));
  if (kind === "tool_started") {
    const toolId = event.toolCallId ?? `${event.tool}-${event.iteration}`;
  ...
  if (kind === "tool_completed" || kind === "tool_failed") {
    ...
    setNodeStates((states) => ({
      ...states,
      tools: event.isError ? "error" : "done",
    }));
    setMessages((current) =>
      updateAssistant(current, assistantId, (message) => ({
        ...message,
...
function toChatMessages(entries: ChatLogEntry[]): ChatMessage[] {
  ...
  for (const [runId, rows] of groups) {
    ...
    const final = [...rows]
      .reverse()
      .find((row) => row.kind === "assistant_message");
    const toolCalls = rows
      .filter((row) => row.kind === "assistant_tool_call")
      .flatMap((row) => blocks(row.content, "tool_use"));
