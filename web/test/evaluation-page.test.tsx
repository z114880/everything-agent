...
it('保留本地启动但不提供或发送记忆快照开关', async () => {
  ...
  expect([...container.querySelectorAll('button')].some(button => button.textContent?.includes('本地直接启动'))).toBe(true);
  expect(container.textContent).toContain('不创建 Langfuse 实验记录');
